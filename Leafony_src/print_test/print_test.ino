...
void initLCD() {
  ...
  // コントラスト = 35 固定
  writeCommand(0x70 | (FIXED_CONTRAST & 0x0F));          // Contrast low
  writeCommand(0x5C | ((FIXED_CONTRAST >> 4) & 0x03));   // Contrast high, Icon off, Booster on
...
void applyState() {
  switch (g_state) {
    ...
    case STATE_CHAR_SAMPLE: /* 未使用 */       break;
  }
}
...
void loop() {
  ...
  if (now - last_sw >= INTERVAL_MS) {
    last_sw = now;
    seq_idx = (seq_idx + 1) & 0x03;   // 0..3 をループ
    g_state = SEQ[seq_idx];
    applyState();
  }
}
